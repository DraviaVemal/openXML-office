...
impl Excel {
    ...
    pub fn new(
        ...
        let root_relations = RelationsPart::new(&rc_office_document, None)
            .context("Initialize Root Relation Part failed")?;
        let content_type = ContentTypesPart::new(&rc_office_document, None)
        ...
        let workbook: WorkbookPart = WorkbookPart::new(&rc_office_document, workbook_path)
        ...
        let core_properties = CorePropertiesPart::new(&rc_office_document, core_properties_path)
    ...
    pub fn add_sheet(&self, sheet_name: String) -> AnyResult<WorkSheetPart, AnyError> {
        let worksheet: Result<WorkSheetPart, AnyError> =
            WorkSheetPart::new(&self.office_document, Some(sheet_name));
        worksheet
    }
